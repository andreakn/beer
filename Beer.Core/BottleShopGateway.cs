...
public class BrewingGateway : IBrewingGateway
{
    ...
    public async Task<(BottleDto, ProblemDetailsDto)> FillBottle(string bottleId)
    {
        var path = $"/api/brewingmachine/fillbottle/{bottleId}";
        var result = await _client.PostAsync(path, null);
        if (result.StatusCode == HttpStatusCode.OK) 
            return (await result.Content.ReadFromJsonAsync<BottleDto>(), null)!;
        if (result.StatusCode == HttpStatusCode.Accepted) 
            return (await result.Content.ReadFromJsonAsync<BottleDto>(), null)!; //TODO - Should indicate non success
        return (null, await result.Content.ReadFromJsonAsync<ProblemDetailsDto>())!;
    }
